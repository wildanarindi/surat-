...
class Sekretariat extends CI_Controller
{
    ...
    public function __construct()
    {
        Parent::__construct();
       is_logged_in();
       $this->load->model('Surkeluar_model', 'keluar', TRUE);
       $this->load->model('Surmasuk_model', 'masuk', TRUE);
    }
...
// Surat Masuk -----------------------------------------------------------------------------------------------------------
	...
	$data = array(
		...
		$this->konten['main_view'] = $this->load->view('sekretariat/masuk', $data, TRUE);
	...
	public function ajax_list_masuk()
	{
		...
		foreach ($list as $us) {
			...
			<a class="dropdown-item btn btn-sm btn-success" href="javascript:void(0)" title="Detail" onclick="detail_masuk('."'".$us->id_tc_surat_masuk."'".')"><i class="fa fa-edit ml-4"></i> Detail</a>';
			...
			// $row[] ='
			// <a class="btn btn-sm btn-warning" href="javascript:void(0)" title="Keterangan" onclick="edit_keterangan('."'".$us->id_tc_surat_masuk."'".')"> Keterangan  </a>';
			$data[] = $row;
		}
...
public function ajax_update_keterangan()
{
	$data = array(
	...
	public function ajax_edit_keterangan($id)
	{
		$data = $this->masuk->get_by_id($id);
		echo json_encode($data);
	}
    ...
    public function keluar()
	{
		...
		$this->konten['main_view'] = $this->load->view('sekretariat/keluar', $data, TRUE);
		$this->load->view('templates/dashboard', $this->konten);
	}
